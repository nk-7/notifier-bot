...
public class R2dbcChatRepository implements ChatRepository {
  ...
  public CompletableFuture<Chat> save(Chat chat) {
    ...
    return Mono.from(connectionFactory.create())
      .flatMap(c ->
        {
          ...
          statement.bind("$2", chat.status().name());
      ...
      .flatMap(row -> Mono.from(row.map((r, md) -> {
        ...
        return new Chat(id, chat.chatId(), chat.title(), chat.type(), chat.status());
      })))
      .onErrorMap(RuntimeException::new)
      .toFuture();
  }

  public CompletableFuture<Optional<Chat>> findByChatId(Long chatId) {
      ...
      .flatMap(row ->
        Mono.from(row.map((r, md) ->
        {
          ...
          return Optional.of(new Chat(
            ...
            ChatStatus.valueOf(r.get("STATUS", String.class))));
        })))
